...
public:
    Piece(const string& name, int hp, int attack, int cost, int vision,
        ...
        : name_(name), hp_(hp), maxHP_(hp), attack_(attack), cost_(cost),
          vision_(vision), speed_(speed), range_(range), pos_{x, y} {}
    ...
    void setMaxHP(int mhp) { maxHP_ = mhp; }
...
public:
    ...
    void heal(Piece* doctor, Player& self) {
        for (size_t i = 0; i < self.getPieces().size(); ++i) {
            ...
            if (dx <= 1 && dy <= 1 && ally != doctor && !dynamic_cast<Home*>(ally)) {
                int nhp = ally->getHP() + doctor->getAttack();
                if (nhp > ally->getMaxHP()) nhp = ally->getMaxHP();
                ally->setHP(nhp);
    ...
    void upgradePiece(Player& player, Piece* p) {
        ...
        int c; if (!(cin >> c)) { cin.clear(); clearInput(); return; }
        clearInput();
        if (c == 1) {
    ...
    void playerTurn(Player& player, Player& enemy) {
        ...
        for (size_t i = 0; i < myPieces.size(); ++i) {
            ...
            while (!acted) {
                ...
                int opAct; if (!(cin >> opAct)) { cin.clear(); clearInput(); continue; }
                clearInput();
                if (!isDoc && opAct == 1) {
                    int tx, ty; cout << "输入目标坐标x y: ";
                    if (!(cin >> tx >> ty)) { cin.clear(); clearInput(); continue; }
                    clearInput();
                    ...
                    if (!(cin >> mx >> my)) { cin.clear(); clearInput(); continue; }
                    clearInput();
    ...
    void runAI() {
        for (int t = 1;; ++t) {
            ...
            if (checkGameOver()) break;
            cout << "\nAI 的回合" << endl;
            const vector<Piece*>& pcs = player2_.getPieces();
            for (size_t i=0;i<pcs.size();++i) {
                Piece* p = pcs[i];
                if (dynamic_cast<Home*>(p)) continue;
                bool acted=false;
                for (size_t j=0;j<player1_.getPieces().size();++j) {
                    Piece* e = player1_.getPieces()[j];
                    int dist = abs(p->getX()-e->getX())+abs(p->getY()-e->getY());
                    if (dist <= p->getRange()) {
                        e->setHP(e->getHP()-p->getAttack());
                        cout << p->getName() << " 攻击 " << e->getName()
                             << " 剩余HP:" << e->getHP() << endl;
                        if (e->getHP() <= 0) {
                            cout << e->getName() << " 被击败" << endl;
                            map_.removePiece(e);
                            player1_.removePiece(e);
                        }
                        acted=true; break;
                    }
                }
                if (!acted) {
                    int tx=player1_.getBaseX();
                    int ty=player1_.getBaseY();
                    int nx=p->getX();
                    int ny=p->getY();
                    if (nx<tx) nx++; else if (nx>tx) nx--;
                    if (ny<ty) ny++; else if (ny>ty) ny--;
                    if (!map_.isOccupied(nx,ny) && map_.inBounds(nx,ny))
                        p->setPosition(nx,ny);
                }
            }
            healPhase(player2_);
...
int main() {
    ...
    srand(time(NULL));
    while (true) {
        cout << "\n1.开始游戏  2.修改职业属性  3.人机对战  4.退出游戏\n选择: ";
        int op; if (!(cin >> op)) { cin.clear(); clearInput(); continue; }
        clearInput();
        if (op == 1) {
            string n1, n2;
            cout << "输入玩家1名称: "; cin >> n1; clearInput();
            cout << "输入玩家2名称: "; cin >> n2; clearInput();
            Game game(n1, n2);
            game.init();
            game.run();
        } else if (op == 2) {
            ...
            string n1; cout << "输入玩家名称: "; cin >> n1; clearInput();
            Game game(n1, "AI");
            game.init();
            game.runAI();
        } else if (op == 4) {
