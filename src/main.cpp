...
public:
    ...
    bool isDefeated() const {
        return (!home_ || home_->getHP() <= 0 || !king_ || king_->getHP() <= 0);
    }
};
...
public:
        ...
        for (size_t i = 0; i < myPieces.size(); ++i) {
            ...
            while (!acted) {
                ...
                if (!isDoc && opAct == 1) {
                    ...
                    if (target->getHP() <= 0) {
                        cout << target->getName() << " 被击败" << endl;
                        bool isOre = dynamic_cast<Ore*>(target) != NULL;
                        map_.removePiece(target);
    ...
    bool checkGameOver() {
        if (player1_.isDefeated()) { cout << player2_.getName() << " 获胜" << endl; return true; }
        if (player2_.isDefeated()) { cout << player1_.getName() << " 获胜" << endl; return true; }
        return false;
    }
...
public:
    ...
    bool checkGameOver() {
        if (human_.isDefeated()) { cout << "AI 获胜" << endl; return true; }
        if (ai_.isDefeated()) { cout << human_.getName() << " 获胜" << endl; return true; }
        return false;
    }
